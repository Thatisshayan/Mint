...
export default async function publishRoutes(fastify: FastifyInstance) {
  fastify.get('/publish', { preHandler: authMiddleware }, async (request: FastifyRequest) => {
  ...
  fastify.post('/publish', { preHandler: authMiddleware }, async (request: FastifyRequest) => {
  ...
  fastify.get('/publish/:id', { preHandler: authMiddleware }, async (request: FastifyRequest) => {
  ...
  fastify.delete('/publish/:id', { preHandler: authMiddleware }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as { id: string };
